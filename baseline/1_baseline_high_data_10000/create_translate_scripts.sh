...
mkdir -p accuracies_and_translate_results translate_scripts
...
for lang in arabic finnish georgian german hindi italian khaling navajo russian turkish; do
	...
	for i in $(seq 1 $1); do
		...
		-model ../models/"$lang"_models/model-"$lang"-high-s"$i"_step_12500.pt \
		-src ../../../language_data/"$lang"/src-"$lang"-dev-1000 \
		-output ../accuracies_and_translate_results/"$lang"_accuracies_and_translate_results/pred_"$lang"_12500_base_high_s"$i".txt \
		-replace_unk \
		-verbose >> translate_scripts/translate_"$lang"_s"$i".sh

		echo "rm -f slurm*" >> translate_scripts/translate_"$lang"_s"$i".sh
		echo "sbatch translate_${lang}_s${i}.sh" >> translate_scripts/all_translate.sh
