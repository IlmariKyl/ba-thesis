...
mkdir -p test_data_accuracies_and_translate_results test_data_translate_scripts
...
for lang in arabic finnish georgian german hindi italian khaling navajo russian turkish; do
    ...
    for i in $(seq 1 $1); do
        ...
        -model ../models/"$lang"_models/model-"$lang"-s"$i"-high_step_12500.pt \
        -src ../../../language_data/"$lang"/src-"$lang"-uncovered-test-1000 \
        -output ../test_data_accuracies_and_translate_results/"$lang"_accuracies_and_translate_results/pred_test_data_"$lang"_12500_base_high_s"$i".txt \
        -replace_unk \
