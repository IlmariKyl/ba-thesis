...
for lang in arabic finnish georgian german hindi italian khaling navajo russian turkish; do
	...
	for i in $(seq 1 $1); do
		...
		-data ../"$lang"-train-medium-1000to1000/data_"$lang"_resample1000_s"$i" \
		-save_model ../models/"$lang"_models/model-"$lang"-medium-res"$i" \
		-world_size 1 \
		-gpu_ranks 0 1 \
		-layers 1 \
		-rnn_size 300 \
		...
		-seed 1 >> train_scripts/train_"$lang"_res"$i".sh
