...
for lang in arabic finnish georgian german hindi italian khaling navajo russian turkish; do
	...
	for i in $(seq 1 $1); do
		...
		-model ../models/"$lang"_models/model-"$lang"-res"$i"-high_step_12500.pt \
		-src ../../../language_data/"$lang"/src-"$lang"-uncovered-test-1000 \
		-output ../test_data_accuracies_and_translate_results/"$lang"_accuracies_and_translate_results/pred_test_data_"$lang"_12500_exp1_res"$i".txt \
		-replace_unk \
