for lang in arabic finnish georgian german hindi italian khaling navajo russian turkish; do
    for file in ../test_data_accuracies_and_translate_results/"$lang"_accuracies_and_translate_results/accuracies/*; do
        if [[ $(basename "$file") == all_accuracies* ]] ; then
            ...
            cat ../test_data_accuracies_and_translate_results/"$lang"_accuracies_and_translate_results/accuracies/all_accuracies.txt | grep -o "base_voted.txt\s\+[0-9\.]\+" | grep -o "[0-9\.]\+$" > "$lang"_voted_test_data_accuracy
            ...
            cat ../test_data_accuracies_and_translate_results/"$lang"_accuracies_and_translate_results/accuracies/all_accuracies.txt | grep -o "weighted_voted.txt\s\+[0-9\.]\+" | grep -o "[0-9\.]\+$" > "$lang"_weighted_voted_accuracy
